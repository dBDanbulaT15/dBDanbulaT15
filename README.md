...
2. **Сравнительная статистика серверов:**
...
4. _Запросы по миру за последний год:_
...
5. Выбор лучшего веб-сервера между Caddy, Nginx и Apache зависит от конкретных требований и предпочтений проекта. Вот краткое сравнение ключевых характеристик каждого из них:
...
Вопрос: Какая цель процессов управления инцидентами? Приведите примеры терминов и дайте им определение (не менее 4)?

Целью процесса управления инцидентами является максимально оперативное восстановление нормальной работы услуги, прописанной в SLA (Соглашение об уровне обслуживания).
В отличии от процесса управления проблемами, который направлен на выяснение и устранение корневой причины для дальнейшего перевода похожих инцидентов в разряд известных ошибок, управление инцидентами,
как было сказано ранее, направлено на быстрое восстановление услуги, которое, в основном, не затрагивает устранение той самой корневой причины возникновения инцидента.
В процессе управления инцидентами производится регистрация инцидента, его классификация и назначение соответствующим группам специалистов, а также мониторинг хода работ по разрешению инцидента, само решение и закрытие инцидента.
Процесс требует постоянное взаимодействие с пользователями, поэтому точкой соприкосновения в процессе является Service Desk.

_Инцидент_ - это событие, которое не относится к стандартному предоставлению услуги,  влечет за собой нарушение работы услуги или ухудшение качества.

_Cтепень воздействия инцидента_ - степень отклонения от нормального уровня предоставления услуги, выражающаяся в количестве пользователей или бизнес-процессов, подвергшихся воздействию инцидента.

_Cрочность инцидента_ - показатель того, насколько оперативно необходимо устранить инцидент; приемлемая задержка разрешения инцидента для пользователя или бизнес-процесса.

_Эскалация инцидента_ - привлечение дополнительных знаний или полномочий для его разрешения. Различают функциональную (горизонтальная) и иерархическую (вертикальная) эскалацию.

_Запрос на Обслуживание_ - это Запрос от Пользователя на поддержку, предоставление информации, консультации или документации.


_Сопутствующие вопросы:_

1. Какая цель у процесса управления проблемами?
2. Что такое проблема и известная ошибка?
3. Виды эскалации инцидентов?
